...
# AJ Kupffer cell analysis using nichenet
# Notebook to run NicheNet regression on AJ specific genes (union set)
...
geneset = "./nichenet_ajUnion.tsv" # pick set of target genes
tpm = 10 # TPM to define an 'expressed' gene
out_prefix = 'aj_kupffer'
out_directory = './AJ/'
...
foreground_set = expressed_genes_aj_kup
niche_set = expressed_genes_aj_niche
...
n_ligands_df = 84 # ligands for df
...
#  Look at cell specificity of best upstream ligands 
best_upstream_ligands %>% intersect(expressed_ligands)
best_upstream_ligands %>% intersect(expressed_genes_aj_hep)
best_upstream_ligands %>% intersect(expressed_genes_aj_lsec)
best_upstream_ligands %>% intersect(expressed_genes_aj_stel)
...
ligand_expression_tbl = tibble(
  ...
  hep = rowMeans(expression[best_upstream_ligands, aj_hep]),
  lsec = rowMeans(expression[best_upstream_ligands, aj_lsec]),
  stel = rowMeans(expression[best_upstream_ligands, aj_stel]))
...
hep_spec_ligands = c()
lsec_spec_ligands = c('BMP2', 'BMP6')
stel_spec_ligands = c('BMP4', 'GDF2', 'NGF', 'VEGFC', 'CXCL12')
hep_lsec_ligands = c()
hep_stel_ligands = c()
lsec_stel_ligands = c('LAMB2', 'ITGB1', 'ADAM17')
general_ligands = c()
